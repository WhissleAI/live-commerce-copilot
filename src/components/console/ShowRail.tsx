  ...
  return (
    <div className="shrink-0">
      <SectionHeader title="Pinned lot">
        {/* eBay Live names lots for the seller — "#007 — As seen on eBay LIVE" —
            so this name is a guess from host speech and a camera frame. When the
            guess is wrong, every answer after it is wrong in the same direction,
            and the operator is the only one who can see that. */}
        <BadgeButton
          onClick={() => {
            setDraft(listing.title);
            setRenaming((v) => !v);
          }}
          title="Tell the copilot what is actually on screen"
        >
          not this lot?
        </BadgeButton>
      ...
      {renaming ? (
        <div className="anim-in flex items-center gap-1.5 px-3 pb-2">
          <input
            autoFocus
            value={draft}
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Enter" && draft.trim().length > 2) {
                onRename(listing.id, draft.trim());
                setRenaming(false);
              }
            }}
            aria-label="What is actually on screen"
            className="min-w-0 flex-1 rounded-sm bg-canvas px-2 py-1.5 text-[12px] focus:outline-none focus:ring-[1.5px] focus:ring-accent"
          />
          <ConsoleButton
            variant="primary"
            onClick={() => {
              if (draft.trim().length > 2) onRename(listing.id, draft.trim());
              setRenaming(false);
            }}
          >
            Name it
...
function AuditRow({ e, onInspect }: { e: AuditEntry; onInspect: () => void }) {
  const tone =
  ...
  return (
    <li className="border-b border-hairline/60">
      ...
      >
        {/* Opens the inspector. It used to expand a raw JSON blob inline — the
            detail is the point, but `{"listingId":"lst_…"}` is a dump, not an
            answer, and it pushed the rest of the chain off screen. */}
        <button
          type="button"
          onClick={onInspect}
          title="Open this entry in the inspector"
...
export function ShowRail({
  ...
  /** Correcting the lot on screen — the one human input that fixes every
   *  answer that would have been wrong after it. */
  onRename: (id: string, title: string) => void;
    ...
    <aside className="flex h-full min-h-0 flex-col bg-panel">
      ...
      <div className="flex min-h-0 flex-1 flex-col">
        ...
        <div className="scroll-thin min-h-0 flex-1 overflow-y-auto">
          <ul>
            {ordered.map((e) => (
              <AuditRow key={e.seq} e={e} onInspect={() => onInspect(e.seq)} />
            ))}
          </ul>
        </div>
      </div>
